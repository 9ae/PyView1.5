...
def set_di_options (parser):
    ...
    import libnidaqmx
    from libnidaqmx import make_pattern
    parser.set_usage ('''\
%prog [options]
...
''')
    ...
    for dev in libnidaqmx.DigitalInputTask.get_system_devices():
...
''')
    phys_channel_choices = []
    for dev in libnidaqmx.DigitalOutputTask.get_system_devices():
...
''')
    ...
    for dev in libnidaqmx.AnalogInputTask.get_system_devices():
...
''')
    ...
    for dev in libnidaqmx.AnalogOutputTask.get_system_devices():
